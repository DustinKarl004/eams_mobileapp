...
const FreshmenParentsInfo = () => {
  const navigation = useNavigation();
  const [fatherLastName, setFatherLastName] = useState('');
  const [fatherFirstName, setFatherFirstName] = useState('');
  const [fatherMiddleName, setFatherMiddleName] = useState('');
  const [fatherSuffix, setFatherSuffix] = useState(''); // Added suffix state
  ...
  const fetchExistingData = async (email) => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        setFatherSuffix(parentsData.father_suffix || ''); // Added suffix fetch
  ...
  const handleNextPress = async () => {
    ...
    const applicantData = {
      ...
      father_suffix: fatherSuffix || '', // Added suffix to save
    ...
    try {
      ...
      setIsLoading(false);
      navigation.navigate('FreshmenIncomeAndBeneficiary');
    } catch (error) {
...
export default FreshmenParentsInfo;
