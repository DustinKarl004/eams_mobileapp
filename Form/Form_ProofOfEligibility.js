...
const ViewProofOfEligibility = () => {
  const navigation = useNavigation();
  const [isLoading, setIsLoading] = useState(true);
  const [proofOfEligibility, setProofOfEligibility] = useState(null);
  ...
  const fetchExistingData = async (email) => {
    try {
      ...
      if (freshmenDocSnap.exists()) {
        ...
        setProofOfEligibility(data.proof_of_eligibility || {});
  ...
  const handleNextPress = () => {
    navigation.navigate('ViewFamilyInformation');
  };
          ...
          <ScrollView contentContainerStyle={styles.scrollContent}>
            <View style={styles.formContainer}>
              ...
              <View style={styles.inputBox}>
                ...
                {proofOfEligibility?.file_urls && proofOfEligibility.file_urls.length > 0 ? (
                  proofOfEligibility.file_urls.map((fileUrl, index) => (
...
export default ViewProofOfEligibility;
