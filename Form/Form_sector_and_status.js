...
const ViewSectorAndWorkStatus = () => {
  const navigation = useNavigation();
  const [isLoading, setIsLoading] = useState(true);
  const [sectorAndWorkStatus, setSectorAndWorkStatus] = useState(null);
  ...
  const fetchExistingData = async (email) => {
    try {
      ...
      if (freshmenDocSnap.exists()) {
        ...
        setSectorAndWorkStatus({
          sector: sector_and_work_status.sector || 'Not specified',
          work_status: sector_and_work_status.work_status || 'Not specified',
          ...(sector_and_work_status.sector === 'Other' && { otherSector: sector_and_work_status.otherSector })
        });
  ...
  const handleNextPress = () => {
    navigation.navigate('ViewParentsInfo');
  };
        ...
        {isLoading ? (
          ...
          <ScrollView contentContainerStyle={styles.scrollContent}>
            <View style={styles.formContainer}>
              ...
              <InfoField 
                label="Sector" 
                value={sectorAndWorkStatus?.sector === 'Other' ? sectorAndWorkStatus?.otherSector : sectorAndWorkStatus?.sector} 
                icon="business-outline" 
              />
              <InfoField 
                label="Work Status" 
                value={sectorAndWorkStatus?.work_status} 
...
export default ViewSectorAndWorkStatus;
