...
const TransfereeProofofResidency = () => {
  ...
  const handleNextPress = async () => {
    ...
    try {
      ...
      const storageRef = ref(storage, `uploads/transferee_proof_of_residency/${userEmail}`);
      ...
      setIsLoading(false);
      navigation.navigate('TransfereeOtherApplicantInfo');
    } catch (error) {
  ...
  const uploadFile = async (file) => {
    try {
      ...
      const storageRef = ref(storage, `uploads/transferee_proof_of_residency/${userEmail}/${fileName}`);
...
export default TransfereeProofofResidency;
